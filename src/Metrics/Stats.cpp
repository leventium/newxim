...
void Stats::FlitRouted(const Flit& flit) { flits_routed++; }
...
std::int32_t Stats::GetBufferFlitsReceived(std::int32_t relay,
                                           std::int32_t vc) {
  auto it = Buffers.find({relay, vc});
...
std::int32_t Stats::GetFlitsRouted() const { return flits_routed; }
