  ...
  if (Timer.SimulationTime() >= 0.0) {
    ...
    if (level > CurrentLevel || percent > CurrentPercent) {
      if (CurrentLevel < 0) {
        ...
        std::string percent_str = std::to_string(percent) + '%';
        std::int32_t start = std::min(
            CurrentLevel,
            BarUnits / 2 - static_cast<std::int32_t>(percent_str.size()) / 2);
...
ProgressBar::ProgressBar(sc_module_name, std::ostream& os,
                         const SimulationTimer& timer, std::int32_t units,
                         const sc_clock& clk)
    : Output(os),
      Timer(timer),
      BarUnits(units),
...
ProgressBar::ProgressBar(std::ostream& os, const SimulationTimer& timer,
                         std::int32_t units, const sc_clock& clk)
