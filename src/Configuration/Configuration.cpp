...
T ReadParam(const YAML::Node& node, const std::string& param) {
  try {
    ...
    throw std::runtime_error(
        (std::stringstream() << "Unable to parse parameter [" << param << "].")
            .str());
  }
}
...
void Configuration::ReadTopologyParams(const YAML::Node& config) {
  ...
  if (topology == "CUSTOM") {
    ...
    graph =
        CirculantGraph(args[0].as<std::int32_t>(), generators, channels_count);
  } else if (topology == "MESH") {
    graph = MeshGraph(dim_x = args[0].as<std::int32_t>(),
                      dim_y = args[1].as<std::int32_t>(), channels_count);
  } else if (topology == "TORUS") {
    graph = TorusGraph(dim_x = args[0].as<std::int32_t>(),
                       dim_y = args[1].as<std::int32_t>(), channels_count);
  } else if (topology == "TREE") {
    graph = TreeGraph(args[0].as<std::int32_t>(), args[1].as<std::int32_t>(),
                      channels_count);
  } else {
    throw std::runtime_error(
        (std::stringstream() << "Unsupported topology [" << topology << "].")
            .str());
  }
  ...
  if (generator != "NONE") {
    ...
    if (subnetwork == "NONE") {
      ...
      throw std::runtime_error((std::stringstream()
                                << "Unsupported subnetwork type [" << subnetwork
                                << "].")
                                   .str());
    }
  } else {
  ...
  try {
    ...
    if (node.IsSequence()) {
        ...
        for (std::int32_t i = 0; i < node.size(); i++) {
          ...
          for (std::int32_t j = 0; j < branch.size(); j++) {
            if (branch[j].IsSequence()) {
              ...
              rnode.push_back(
                  std::vector<std::int32_t>(1, branch[j].as<std::int32_t>()));
            }
          }
          table.push_back(std::move(rnode));
        }
      ...
      if (type == "DIJKSTRA") {
        ...
        throw std::runtime_error((std::stringstream()
                                  << "Unsupported routing_table [" << type
                                  << "].")
                                     .str());
      }
    }
...
void Configuration::ReadSimulationParams(const YAML::Node& config) {
  ...
  report_topology_graph_adjacency_matrix =
      ReadParam<bool>(config, "report_topology_graph_adjacency_matrix");
  ...
  report_topology_sub_graph =
      ReadParam<bool>(config, "report_topology_sub_graph");
  report_topology_sub_graph_adjacency_matrix =
      ReadParam<bool>(config, "report_topology_sub_graph_adjacency_matrix");
  report_sub_routing_table =
      ReadParam<bool>(config, "report_sub_routing_table");
  ...
  if (production_time < 1 || production_time > simulation_time) {
    throw std::runtime_error(
        "production_time can not be less than 1 or grater than "
        "simulation_time.");
  }
  ...
  if (max_packet_size < min_packet_size) {
    throw std::runtime_error(
        "max_packet_size can not be less than min_packet_size.");
  }
}
void Configuration::ReadTrafficDistributionParams(const YAML::Node& config) {
  ...
  if (traffic_distribution == "TRAFFIC_TABLE_BASED")
    traffic_table_filename =
        ReadParam<std::string>(config, "traffic_table_filename");
    ...
    for (std::int32_t i = 0; i < traffic_hotspots.size(); i++) {
      ...
      hotspots.push_back(
          std::make_pair(hotspot[0].as<std::int32_t>(),
                         std::make_pair(hotspot[1].as<std::int32_t>(),
...
void Configuration::ReportData() {
    ...
    for (std::int32_t s = 0; s < graph.size(); s++) {
      for (std::int32_t d = 0; d < graph.size(); d++) {
        ...
        std::cout << std::setfill('0') << "[" << std::setw(2) << s << " -> "
                  << std::setw(2) << d << "]:\n";
        for (std::int32_t i = 0; i < static_cast<std::int32_t>(routes.size());
             i++) {
          const auto& route = routes[i];
          for (std::int32_t j = 0;
               j < static_cast<std::int32_t>(route.size()) - 1; j++) {
            std::cout << route[j] << " -> ";
          }
          if (route.size()) {
            std::cout << route.back();
  ...
  if (report_routes_stats) {
    ...
    std::cout << "Average min route:  "
              << static_cast<double>(total_min_distance) /
...
Configuration::Configuration(std::int32_t arg_num, char* arg_vet[]) {
  ...
  try {
    ...
    std::cout << " Failed\n";
    throw std::runtime_error(
        "The specified YAML configuration file was not found!\nUse -config to "
        "load examples from config_examples folder.\n");
  } catch (YAML::ParserException& pe) {
    std::cout << " Failed\n";
    throw std::runtime_error((std::stringstream()
                              << "ERROR at line [" << pe.mark.line + 1
                              << "] column [" << pe.mark.column + 1
                              << "]: " << pe.msg << ".")
                                 .str());
  }
...
void Configuration::ParseArgs(YAML::Node& node, std::int32_t arg_num,
                              char* arg_vet[]) {
  for (std::int32_t i = 1; i < arg_num; i++) {
    if (arg_vet[i][0] == '-') {
      if (i + 1 >= arg_num) {
      ...
      throw std::runtime_error(
          (std::stringstream() << "Invalid option [" << arg_vet[i] << "].")
              .str());
    }
  }
}

const Graph& Configuration::TopologyGraph() const { return graph; }
const Graph& Configuration::TopologySubGraph() const { return subgraph; }
const Graph& Configuration::NetworkGraph() const { return network_graph; }
const RoutingTable& Configuration::GRTable() const { return table; }
const RoutingTable& Configuration::SubGRTable() const { return subtable; }
const std::vector<
    std::pair<std::int32_t, std::pair<std::int32_t, std::int32_t>>>&
Configuration::Hotspots() const {
  return hotspots;
}
...
std::int32_t Configuration::BufferDepth() const { return buffer_depth; }
std::int32_t Configuration::MinPacketSize() const { return min_packet_size; }
std::int32_t Configuration::MaxPacketSize() const { return max_packet_size; }
...
double Configuration::PacketInjectionRate() const {
  if (!flit_injection_rate)
    return packet_injection_rate / (scale_with_nodes ? graph.size() : 1.0);
  else
    return packet_injection_rate * 2.0 / (min_packet_size + max_packet_size) /
           (scale_with_nodes ? graph.size() : 1.0);
}
double Configuration::Locality() const { return locality; }
...
std::int32_t Configuration::ClockPeriodPS() const { return clock_period_ps; }
std::int32_t Configuration::SimulationTime() const { return simulation_time; }
std::int32_t Configuration::ProductionTime() const { return production_time; }
std::int32_t Configuration::ResetTime() const { return reset_time; }
...
bool Configuration::ReportProgress() const { return report_progress; }
bool Configuration::JsonResult() const { return json_result; }
bool Configuration::ReportBuffers() const { return report_buffers; }
bool Configuration::ReportCycleResult() const { return report_cycle_result; }
bool Configuration::ReportFlitTrace() const { return report_flit_trace; }
bool Configuration::ReportDistribution() const { return report_distribution; }
double Configuration::FlitTraceStart() const { return flit_trace_start; }
double Configuration::FlitTraceEnd() const { return flit_trace_end; }

std::int32_t Configuration::DimX() const { return dim_x; }
std::int32_t Configuration::DimY() const { return dim_y; }
std::int32_t Configuration::ChannelsCount() const { return channels_count; }
