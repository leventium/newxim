...
void Relay::SetLocalID(std::int32_t id) { local_id = id; }
...
bool Relay::CanSend(const Flit& flit) const {
  return tx_current_level == tx_ack.read() &&
...
bool Relay::CanReceive() const { return rx_req.read() == !rx_current_level; }
Flit Relay::Receive() {
  if (CanReceive()) {
    ...
    if (flit.vc_id < 0 || flit.vc_id >= num_virtual_channels)
      throw std::runtime_error("Relay error: Invalid virtual channel [" +
                               std::to_string(flit.vc_id) +
                               "] of received flit.");

    auto& buffer = buffers[flit.vc_id];

    if (buffer.Full())
      throw std::runtime_error("Relay error: Buffer overflow.");
...
Flit Relay::Front() const {
  std::size_t i = 0;
  while (i < num_virtual_channels &&
         buffers[(current_virtual_channel + i) % num_virtual_channels].Empty())
    i++;

  if (i < num_virtual_channels) {
    return buffers[(current_virtual_channel + i) % num_virtual_channels]
        .Front();
  } else {
    return Flit();
  }
...
void Relay::Skip() {
  current_virtual_channel =
      (current_virtual_channel + 1) % num_virtual_channels;
}
Flit Relay::Pop() {
  ...
  std::size_t i = 0;
  while (i < num_virtual_channels &&
         buffers[(vc_offset + i) % num_virtual_channels].Empty())
    i++;
